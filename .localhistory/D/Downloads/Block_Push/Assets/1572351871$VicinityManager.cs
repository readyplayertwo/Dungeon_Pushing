...
public class VicinityManager : MonoBehaviour
{
    ...
    private IEnumerator OnTriggerEnter(Collider other)
    {
        ...
        if (other.name == "HandTrackedBlock" && reachedEnd == true)
        {
            // Use a proxy object, attached to the hand, to actuate the button.
            _tempObject = other.gameObject;

        }
        if (other.name == "HandTrackedBlock" && reachedEnd == false)
        {
            CE._emitter.start();
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.name == "HandTrackedBlock")
        {
            ...
            Block.transform.position = new UnityEngine.Vector3(
                ...
                _pistonPresser.transform.position.y,
                _buttonPositionY);
        }
    ...
    {
        float newZ = Mathf.Clamp(_buttonPositionY, _buttonMinYValue, _buttonMaxYValue);
        Block.transform.position = new UnityEngine.Vector3(Block.transform.position.x, Block.transform.position.y, newZ);
    }
    ...
    private void pressComplete()
    {
        ...
        if (this.gameObject.name == "Block1")
        {
            IsOnePushed = true;
        }
        if (this.gameObject.name == "Block2")
        {
            IsTwoPushed = true;
        }
        if (this.gameObject.name == "Block3")
        {
            IsThreePushed = true;
        }
        if (this.gameObject.name == "Block4")
        {
            IsFourPushed = true;
        }
